...
// MixinImproveDebugging is a spell.Mixin for golang.Options to add linker flags to improve the debugging of binary
// artifacts.
// This includes the disabling of inlining and all compiler optimizations tp improve the compatibility for debuggers.
//
...
//
// Run `go help build`, `go tool compile -help` for the documentation of supported flags.
...
func (m MixinImproveDebugging) Apply(so spell.Options) (spell.Options, error) {
	goOpts, ok := so.(*Options)
	if !ok {
		return nil, errors.New("unsupported spell options type")
	}
...
// MixinImproveEscapeAnalysis is a spell.Mixin for golang.Options to add linker flags to improve the escape analysis of
// binary artifacts.
...
// Apply applies the mixin to the given spell.Options that must be of type golang.Options.
func (m MixinImproveEscapeAnalysis) Apply(so spell.Options) (spell.Options, error) {
	goOpts, ok := so.(*Options)
	if !ok {
		return nil, errors.New("unsupported spell options type")
	}
...
// MixinInjectBuildTimeVariableValues is a spell.Mixin for golang.Options to inject build-time values through the `-X`
...
// Apply applies the mixin to the given spell.Options that must be of type golang.Options.
func (m MixinInjectBuildTimeVariableValues) Apply(so spell.Options) (spell.Options, error) {
	if m.GoModuleID == nil {
	...
	if m.Data != nil {
		for k, v := range m.Data {
			goOpts.LdFlags = append(goOpts.LdFlags, fmt.Sprintf("-X %s/%s=%s", m.GoModuleID.Path, k, v))
		}
	}
...
// MixinStripDebugMetadata is a spell.Mixin for golang.Options to add linker flags to strip debug information from
// binary artifacts.
...
// Apply applies the mixin to the given spell.Options that must be of type golang.Options.
func (m MixinStripDebugMetadata) Apply(so spell.Options) (spell.Options, error) {
	goOpts, ok := so.(*Options)
	if !ok {
		return nil, errors.New("unsupported spell options type")
	}
